...
func TestIsOptionalIPv6Loopback(t *testing.T) {
	...
	if !isOptionalIPv6Loopback("[::1]:6000", errors.New("listen tcp [::1]:6000: bind: cannot assign requested address")) {
		t.Fatal("expected IPv6 loopback unavailable failure to be optional")
	}
	if !isOptionalIPv6Loopback("[::1]:6000", syscall.EADDRNOTAVAIL) {
		t.Fatal("expected wrapped IPv6 loopback unavailable errno to be optional")
	}
